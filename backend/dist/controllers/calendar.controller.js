...
export const updateEvent = async (req, res) => {
    try {
        ...
        const event = await Event.findByIdAndUpdate(id, { ...updates, date: updates.date ? new Date(updates.date) : undefined }, { new: true, runValidators: true });
        if (!event) {
            return res.status(404).json({
                success: false,
                message: 'Event not found',
...
export const updateHoliday = async (req, res) => {
    try {
        const { id } = req.params;
        const updates = req.body;
        const holiday = await Holiday.findByIdAndUpdate(id, { ...updates, date: updates.date ? new Date(updates.date) : undefined }, { new: true, runValidators: true });
        if (!holiday) {
            return res.status(404).json({
                success: false,
                message: 'Holiday not found',
