 ...
 */
export const createWFH = async (req: Request, res: Response) => {
  try {
 ...
 */
export const getWFHDays = async (req: Request, res: Response) => {
  try {
 ...
 */
export const deleteWFH = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const wfh = await WorkFromHome.findByIdAndDelete(id);
    if (!wfh) return res.status(404).json({ success: false, message: 'WFH day not found' });
...
export const getCalendarData = async (req: Request, res: Response) => {
  try {
    ...
    // Get holidays
    const holidayQuery: any = year && month ? { date: dateFilter } : {};
    ...
    if (year && month) wfhQuery.date = dateFilter;
    ...
    if (userEmail && userRole !== 'Admin') {
      const leaveQuery: any = { email: userEmail, status: 'Approved' };
 ...
 */
export const getAdminCalendarData = async (req: Request, res: Response) => {
  try {
    const { year, month } = req.query;
    ...
    const wfhQuery: any = year && month ? { date: dateFilter } : {};
