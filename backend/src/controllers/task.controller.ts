...
const ROLE_MODEL_MAP: Record<'Student' | 'Intern' | 'Employee', any> = {
  Student,
  Intern,
  Employee,
};

const getModelByAssigneeType = (assigneeType: 'Student' | 'Intern' | 'Employee') => ROLE_MODEL_MAP[assigneeType];
...
export const createTask = async (req: Request, res: Response) => {
  try {
    ...
    if (roleFilter && assignedToType === 'Student') filterQuery.course = roleFilter;
    if (roleFilter && assignedToType === 'Employee') filterQuery.designation = roleFilter;
    if (roleFilter && assignedToType === 'Intern') filterQuery.department = roleFilter;
    ...
    const task = new Task({
      ...
      description,
      assignedBy,
      assignedTo: assigneeRecords.map((assignee: any) => assignee._id),
      assignedToType,
    ...
    const recipientEmails = assigneeRecords
      .map((assignee: any) => assignee.email)
      .filter(Boolean);
    ...
    console.log(
      `✓ Task created: ${task._id} - Assigned to ${assigneeRecords.length} ${assignedToType}(s) [${assignToAll ? 'All' : 'Selected'}]`
    );
...
export const getTasksByAssignee = async (req: Request, res: Response) => {
  try {
    ...
    if (['Student', 'Intern', 'Employee'].includes(userRole)) {
      ...
      assigneeId = (profile._id as any).toString();
    } else {
    ...
    const query: any = { assignedTo: assigneeId, assignedToType: assigneeType };
...
export const getAllTasks = async (req: Request, res: Response) => {
  try {
    ...
    if (assignedToType) query.assignedToType = assignedToType;
...
export const updateTaskStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (status === 'Submitted') {
      const alreadySubmitted = task.taskSolutions?.some(
        (solution: any) => solution.assigneeId?.toString() === userId
      );
      if (!alreadySubmitted) {
 ...
 */
export const submitTaskSolution = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const profileModel = getModelByAssigneeType(userRole as 'Student' | 'Intern' | 'Employee');
    const assigneeProfile = await profileModel.findOne({ email: (req as any).user?.email }).select('_id');
    ...
    const isAssignee = task.assignedTo.some((id: any) => id.toString() === assigneeId);
    if (!isAssignee) {
      return res.status(403).json({
        success: false,
    ...
    const existingIndex = task.taskSolutions.findIndex(
      (solution: any) => solution.assigneeId?.toString() === assigneeId
    );
 ...
 */
export const reviewTaskSolution = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const solution = task.taskSolutions.find((entry: any) => entry.assigneeId?.toString() === assigneeId);
    if (!solution) {
...
export const getTasksAssignedByUser = async (req: Request, res: Response) => {
  try {
    ...
    const { status, priority, assignedToType } = req.query;
    const query: any = { assignedBy: userId };
